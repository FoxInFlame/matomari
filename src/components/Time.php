...
class Time
{
   ...
   * @since 0.5
   */
  public static function getAbsoluteTimeGMT($string, $defaultFormat = '!M j, g:i A') {
    ...
    if(strpos($string, 'ago') !== false) {
      ...
      $date = date_create_from_format('g:i A', substr($string, 7), new \DateTimeZone('Etc/GMT+8'));
      $date->setTimeZone(new \DateTimeZone('Etc/GMT'));
      return $date;
    } else if(strpos($string, 'Yesterday') !== false) {
      $date = date_create_from_format('g:i A', substr($string, 11), new \DateTimeZone('Etc/GMT+8'));
      $date->setTimeZone(new \DateTimeZone('Etc/GMT'));
      $date->modify('-1 day');
      return $date;
    } else {
      $date = date_create_from_format($defaultFormat, $string, new \DateTimeZone('Etc/GMT+8'));
      $date->setTimeZone(new \DateTimeZone('Etc/GMT'));
