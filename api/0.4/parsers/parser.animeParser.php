...
class AnimeParser {
  ...
  public static function parse($response) {
    ...
    $anime->set("id", $html->find("div#contentWrapper #editdiv input[name=\"aid\"]", 0)->value);

    // The Title
    // <span itemprop="name">The Last: Naruto the Movie</span>
    $anime->set("title", $html->find("div#contentWrapper div h1.h1 span", 0)->innertext);

    // The MAL URL
    // <a href="https://myanimelist.net/anime/16870/The_Last__Naruto_the_Movie" class="horiznav_active">Details</a>
    $anime->set("mal_url", $html->find("div#contentWrapper #content #horiznav_nav ul li a", 0)->href);
    ...
    if($html->find("div#contentWrapper div#content table div a img.ac", 0)) {
      ...
      $anime->set("image_url", $imageSource);
    }
    ...
    if(trim($html->find("div#contentWrapper div#content div.anime-detail-header-stats .score", 0)->plaintext) !== "N/A") {
      $anime->set("score", trim($html->find("div#contentWrapper div#content div.anime-detail-header-stats .score", 0)->plaintext));
    }
    ...
    if($html->find("div#contentWrapper div#content div.anime-detail-header-stats span.ranked strong", 0)->plaintext != "N/A") {
      $anime->set("rank", substr($html->find("div#contentWrapper div#content div.anime-detail-header-stats span.ranked strong", 0)->plaintext, 1));
    }
    ...
    foreach($html_information as $value) {
      ...
      if(strpos($value->plaintext, "Type:") !== false) {
        ...
        if(strpos($value->plaintext, "Unknown") === false) {
          $anime->set("type", trim($value->find("a", 0)->innertext));
        }
      }
      ...
      if(strpos($value->plaintext, "Episodes:") !== false) {
        if(strpos($value->innertext, "Unknown") === false) {
          $anime->set("episodes", trim($value->find("text", 2)->innertext));
        }
      }
      ...
      if(strpos($value->plaintext, "Status:") !== false) {
        if(strpos($value->innertext, "Unknown") === false) {
          $anime->set("air_status", trim($value->find("text", 2)->innertext));
        }
      }
      ...
      if(strpos($value->plaintext, "Aired:") !== false) {
        if(strpos($value->innertext, "Unknown") === false) {
          if(strpos($value->find("text", 2)->innertext, " to ") !== false) {
            ...
            if($exploded[0] !== "?") {
              $anime->set("air_date_from", getAbsoluteTimeGMT($exploded[0], "!M j, Y")->format("c"));
            }
            if($exploded[1] !== "?") {
              $anime->set("air_date_to", getAbsoluteTimeGMT($exploded[1], "!M j, Y")->format("c"));
            }
          } else {
            echo $value->find("text", 2)->innertext;
            $anime->set("premier_date", getAbsoluteTimeGMT($value->find("text", 2)->innertext, "!M j, Y")->format("c"));
          }
        }
      }
      ...
      if(strpos($value->plaintext, "Premiered:") !== false) {
        if(strpos($value->plaintext, "Unknown") === false) {
          $anime->set("season", trim($value->find("a", 0)->innertext));
        }
      }
      ...
      if(strpos($value->plaintext, "Broadcast:") !== false) {
        if(strpos($value->plaintext, "Unknown") === false) {
          $anime->set("air_time", trim($value->find("text", 2)->innertext));
        }
      }
      ...
      if(strpos($value->plaintext, "Source:") !== false) {
        if(strpos($value->innertext, "Unknown") === false) {
          $anime->set("source", trim($value->find("text", 2)->innertext));
        }
      }
      ...
      if(strpos($value->plaintext, "Duration:") !== false) {
        if(strpos($value->plaintext, "Unknown") === false) {
          ...
          if(strpos($value->plaintext, " per ep.") !== false) {
            $anime->set("duration_per_episode", $minutes);
            if($anime->get("episodes") == null) {
              $anime->set("duration_total", null);
            } else {
              $anime->set("duration_total", intval($anime->get("episodes")) * intval($minutes));
            }
          }
        }
      }
      ...
      if(strpos($value->plaintext, "Rating:") !== false) {
        if(strpos($value->plaintext, "Unknown") === false) {
          $anime->set("rating", trim($value->find("text" , 2)->innertext));
        }
      }
      ...
      if(strpos($value->plaintext, "Score:") !== false) {
        if(strpos($value->plaintext, "users") !== false) {
          $anime->set("members_scored", trim(str_replace(",", "", $value->find("span[itemprop=\"ratingCount\"]", 0)->innertext)));
        }
      }
      ...
      if(strpos($value->plaintext, "Members:") !== false) {
        $anime->set("members_inlist", trim($value->find("text", 2)->innertext));
      }
      ...
      if(strpos($value->plaintext, "Favorites:") !== false) {
        $anime->set("members_favorited", trim($value->find("text", 2)->innertext));
      }
    ...
    $anime->set("background", trim($background));
